...
use App\Domain\API\Voucher\ReactivateRequest as ValidRequest;
...
final class Reactivate {
    ...
    public function __invoke(Request $request, Response $response, array $args): response {
        ...
        try {
            ...
            $this->logger->info("Duty Free voucher reactivation request: " . json_encode($requestdata), $requestdata['jwt']['user']);
            ...
            $this->logger->info("Voucher reactivation started.", $requestdata['jwt']['user']);
            
            $return = $this->repository->reactivate($validRequest);

            $this->logger->info("Voucher reactivation complete", $requestdata['jwt']['user']);

            $response = $response->withJson($return)->withStatus(201);
        } catch (\Exception $e) { //Capture any Exceptions being thrown
            $this->logger->error($e->getMessage(), json_encode($data), $requestdata['jwt']['user']);
            ...
            $this->logger->critical($e->getMessage(), json_encode($data), $requestdata['jwt']['user']);
            $response = $response->withJson(
                    [
                        "error" => "ERR" . $e->getCode(),
                        "message" => $e->getMessage()
