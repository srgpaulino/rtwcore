...
class Zinio extends Provider
{
     ...
     * @var array
     */
    protected $_redownload = array('emagazines' => true);
    ...
    public $countryCodeByTerritory = array(
        // map international to US for country code
        'ALL'  => 'US'
    );

    private $newsstandsByTerritory = array(
        'US' => 101,
        'ALL' => 101,
        'GB' => 120,
        'IE' => 120,
     ...
     * Download content
     *
     * @param string $content Content name (emagazines)
    ...
    private function getToken($clientSecret, $realmName, $grantType = 'client_credentials')
    {
        ...
        $response = json_decode($response, true);
            ...
            //$values['price'] = $data['product']['issues'][$typeId]['price_'.$currency];
            $data['product']['type'] = 'single';
            //$values['id'] = $data['product']['issues'][$typeId]['provider_issue_product_id'];
            //$values['issue_id'] = $data['product']['issues'][$typeId]['provider_issue_id'];
            $data['product']['log_type'] = 'issue';

            $result = $this->placeOrder($data);
            d("place order result: ");
            d($result);
            if(!$result){
                return false;
            }
            ...
            $data['product']['content_type'] = $content;
            $data['product']['sf_content_type'] = ucfirst($type);
            $data['product']['artist'] = $data['product']['publisher'];

            // Add whether the user existed or not, in case we want to display different success messages
            $data['user']['user_created'] = $this->zinioUserCreated;

            // The download history as it is at the moment is for the product rather than offer or issue ordered
            // For now storing the offer or issue data in the additional_info field so that can be referenced if needed
            $additionalInfo = $type == 'offer' ? $data['product']['offers'][$typeId] : $data['product']['issues'][$typeId];
            $data['additional_info'] = "'" . json_encode($additionalInfo) . "'";

            //order data for api return
            $token = $this->getReaderInfo($data);
            $data['order']['accessToken'] = $token;
            $data['order']['zinioUserId'] = $result['user_id'];
            $data['order']['partnerDomain'] = $this->_auth['partnerDomain'];
            $data['order']['issueId'] = $data['product']['issues'][$typeId]['provider_issue_id'];

            $downloadHistoryData = $data['product'];

            return $this->addDownloadHistory($data['product']);
    ...
    private function mclog($status, $message, $orderId=NULL, $errorType=NULL) {
        ...
        $this->logger->message("Redemption started");        
        $this->logger->logIt();
    }
